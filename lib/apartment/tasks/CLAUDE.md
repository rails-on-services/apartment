...
`apartment:migrate` delegates to `Apartment::Migrator`, reading `parallel_migration_threads` and `migration_db_config` from `Apartment.config`. When `parallel_migration_threads > 0`, migration runs across a thread pool of that size. After a successful run, schema dump is triggered only if `ActiveRecord.dump_schema_after_migration` is true (via `db:schema:dump` rake task). Failed tenants abort the task with a non-zero exit.
