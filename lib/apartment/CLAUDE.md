...
An unresolvable role is translated here: `ActiveRecord::ConnectionNotDefined` / `ConnectionNotEstablished` becomes an `Apartment::ConfigurationError` naming `ddl_role` and the symbol given. The `entered` flag is load-bearing — `connected_to` calls the block, so the same rescue sees a connection error raised by a query *inside* it, and translating that would report the caller's own failure as a misconfigured `ddl_role`. Checked at first use rather than at `activate!`, which runs in `after_initialize`, after the eager-load initializer: under lazy loading no model has run `connects_to` yet and a boot-time check would fail on every boot.
