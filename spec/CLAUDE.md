...
- `PG_UNIT_REQUIRED=1` — `spec/unit/transaction_taint_spec.rb` (needs the `pg` gem to name `PG::PQTRANS_*`), `spec/unit/patches/postgresql_sequence_name_spec.rb` (needs `PostgreSQL::Utils`), `spec/unit/patches/postgresql_type_map_spec.rb` (needs the real `HashLookupTypeMap` and the adapter file) and the `.apply!` example in `spec/unit/schema_dumper_patch_spec.rb` (needs `PostgreSQL::SchemaDumper`). These are PostgreSQL-only, so they skip in the default driverless unit bundle and run under the PostgreSQL appraisals. Two traps here, both found by audit. First, the `unit` CI job's `BUNDLE_GEMFILE` is a **sqlite3** appraisal and the `postgresql` job used to run only `spec/integration/v4/`, so nothing in CI ever executed these — the flag was documented but set nowhere. The `postgresql` job now has a `Run PG-gated unit specs` step that sets it. Second, having `pg` in the bundle is **not** enough: AR requires the PostgreSQL adapter file lazily, when something opens a PG connection, and the unit suite never does. A spec needing `ActiveRecord::ConnectionAdapters::PostgreSQL::*` must `require('active_record/connection_adapters/postgresql_adapter')` itself, or its `defined?` guard is false even under a postgresql appraisal — which is how `schema_dumper_patch_spec`'s `.apply!` example passed while asserting nothing on all fourteen matrix lanes.
