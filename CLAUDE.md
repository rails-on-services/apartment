...
- `docs/designs/postgresql-type-map-sharing.md` — One PostgreSQL OID type map per database, shared across tenant pools (shipped; see Gotchas)
...
**CI matrix**: Ruby 3.3/3.4/4.0 × Rails 8.1/main × PG 16+18, MySQL 8.4, SQLite3. Rails main is a canary (`continue-on-error`). See `.github/workflows/ci.yml`.
...
- **The PostgreSQL OID type map is shared per database, not per connection** (design: `docs/designs/postgresql-type-map-sharing.md`): Rails rebuilds it on every new connection through `configure_connection` -> `reload_type_map`, three `pg_type` queries, two of them sequential scans of a catalog that grows by two rows per table per tenant schema (65 ms per connect at 500 schemas, measured). Pool-per-tenant pays that on every cold tenant pool, which is why the two statements topped the nightly load on a v4 lane and were absent from v3's. `Apartment::Patches::PostgresqlTypeMap` (gem-load `on_load`, alongside the sequence-name patch) adopts a process-wide `SharedTypeMap` on `clear_cache!(new_connection: true)` + `reload_type_map`, and rebuilds-and-republishes only on the enum DDL helpers' explicit reload, never clearing a published map in place. The key is `[[host, port, database], [database_oid, postmaster_start_time], default_timezone]`, read off the **live connection rather than `@config`** because libpq resolves defaults the config cannot show, and carrying a catalog identity (`pg_database.oid` **plus** the epoch of `pg_postmaster_start_time()`, one 0.096 ms query; the epoch rather than the timestamptz, whose text form follows the session's `TimeZone`/`DateStyle` and would split the key for one catalog) because **a name is not a catalog**: the patch survives `clear_cache!`, which is what heals a replaced catalog today, so a different cluster behind an unchanged endpoint would otherwise be adopted, and fresh catalogs restart type OIDs at 16384 — a stale entry then describes a *different* type instead of going unused. The OID alone is not enough and the counterexample is ordinary, not exotic: `postgres` is OID 5 on every cluster ever initdb'd, so an app on the RDS default database would have a constant identity check. Publishing supersedes same-endpoint entries with a different identity, so restarts do not strand a map each. A probe failure warns, detaches, and falls back to a private map — detaching matters because upstream's `reload_type_map` clears a live map *in place*, which on an adopted instance would blank it for every other holder. Its mapping is a `Concurrent::Map`; no registry lock is held across the catalog queries (`put_if_absent`, not `compute_if_absent`); `apply!` fails closed if either public seam or the private `type_map` reader is gone. `add_pg_decoders`' one per-connection lookup is deliberately untouched. Rails main (rails/rails#57013, unreleased) removes the connect-time queries itself; the patch bridges 7.2 to 8.1 and on main only shares the deferred first-unknown-OID scan. Retention is measured, not bounded: 88.5 KB per map, one key per process under schema-per-tenant, ~49 MB for 570 tenant databases; no size cap ships because one tight enough to matter would thrash the sweep it speeds up, and the only removal is supersede. Config-only mitigations cannot substitute: an LRU cap below the tenant count evicts every pool each pass regardless of `pool_idle_timeout`, and lifting the cap costs `tenant_count × tenant_pool_size` connections per process.
