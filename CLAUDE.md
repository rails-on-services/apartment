...
- **`Model.sequence_name` memoization**: Rails memoizes it per model class, process-wide, resolved schema-qualified on whichever tenant's connection touched it first. `Apartment::Patches::PostgresqlSequenceName` (registered at gem load in `lib/apartment.rb`, not in `activate!`, because memoization can fire during boot) strips the pool's own schema prefix so prefetched ids — e.g. `activerecord-import`'s literal `nextval(Model.sequence_name)` — come from the current tenant's sequence. Foreign prefixes (persistent schemas, pinned models' default-tenant qualification) are preserved on purpose. Nothing inside the gem calls `sequence_name` — host-app gems do — so don't delete the patch as "unused"; that is exactly how the v3 original was lost in #356.
