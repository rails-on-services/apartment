...
What it issues, and where. Before the schema import: `GRANT USAGE ON SCHEMA`, plus three `ALTER DEFAULT PRIVILEGES FOR ROLE <db_role> … GRANT` rules covering tables, sequences and functions. After it: `GRANT … ON ALL TABLES` and `ON ALL SEQUENCES`, which cover the objects the import created. `FOR ROLE` names `ctx.db_role` — the **database** principal `ddl_role` resolves to, not the ActiveRecord role symbol, which is not a thing PostgreSQL can be told about. It is explicit rather than implied by whoever executed the statement, so the rule is correct by statement rather than by position.
