...
- **`PQTRANS_INERROR` recovery hooks** (failure-class member 7) — ✅ **CONFIRMED REAL (2026-07-12); promoted out of the wishlist. This is W1, and it is the only internal code work left before beta.** The adopter's `ROLLBACK` loop checks `raw_connection.transaction_status == PG::Connection::PQTRANS_INERROR` and rolls back every writing-pool connection after each cross-tenant example. Two things settle it: `git log -S` dates the loop to their **v4 migration** (so it is not a v3 leftover surviving out of inertia), and it is **test-only** — no production path wraps `Tenant.switch`/`switch!` with it. They also tried widening the predicate app-side, hit a savepoint regression, and concluded in writing that the fix belongs in the gem. Gem-side coverage = instrumented detection + a recovery path in `Apartment::Tenant.switch`'s ensure block, plus an integration spec. Their workaround is the thing it should let them delete.
