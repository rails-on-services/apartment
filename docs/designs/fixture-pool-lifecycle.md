...
- **Within-process thread / job boundaries** (failure-class member 9) — ✅ **AUDITED CLEAN (2026-07-12); downgraded from a suspected failure class to a documentation + helper task (W3).** The adopter runs **none** of the risky patterns: Sidekiq is faked in specs (the two inline opt-ins run synchronously on the example's own thread), CI shards across **OS processes** rather than `parallel_tests` threads, and there are **zero** occurrences of `Tenant.switch`/`switch!` inside `Thread.new`, `Concurrent::Future`, or similar. Their only async executor never touches ActiveRecord. Nothing has bitten — from a codebase that would have surfaced it.
  ...
  What the audit *did* surface is an **API gap, not a bug**: their one pool-aware call site hand-rolls an eviction of the stale per-tenant pool (`Apartment.pool_manager&.remove_tenant`) before switching, on the main thread. That is the real ergonomics signal. Shape the contract and helper around **that**, not around the threading hazard we hypothesized.
