...
| 5 | Non-`:writing` roles / replicas (lifecycle) | Role axis closed; multi-DB axis open | AR snapshots only `connection_pool_list(:writing)` at fixture setup, so a `:reading` tenant pool enrolls solely via the lazy `!connection.active_record` subscriber. `fixture_pool_lifecycle_spec.rb`'s `:reading` context + both-roles example prove that pool pins, trips the guard, and rebuilds with fresh identity per handler (distinct, independently-pinned objects). Same-physical-DB second role; the multi-physical-DB axis (a `:reading` role on a separate database via `connects_to`) is the open residual. See `docs/designs/reading-role-test-support.md`. |
...
| 7 | `PQTRANS_INERROR` taint | **Closed** — checkin heal shipped | A statement Rails did not wrap fails while a transaction Rails will not unwind stays open. Reachable in **production**, not only under fixtures: AR's `active?` probes with an empty query (which does not error in an aborted transaction), so `verify!` calls a poisoned connection healthy and the pool serves it to the next caller — and under pool-per-tenant that is the tenant's *only* connection. Healed at pool checkin via `conn.reset!`; fixture-pinned connections are skipped, because their transaction belongs to `teardown_fixtures`. PostgreSQL-only. Design: [`transaction-taint-detection.md`](transaction-taint-detection.md). |
| 8 | Schema cache / prepared-statement drift after tenant DDL | Reactive recovery shipped | v4's pool-per-tenant gives each tenant its own schema cache (no cross-tenant drift) and AR self-heals PG prepared statements; the residual shared/pinned-table-DDL amplifier is handled by the manual `Apartment::Tenant.reload_schema_cache!` recovery helper (current-process). Cross-process proactive invalidation stays deferred (transport seam). See `docs/designs/v4-schema-cache-recovery.md`. |
...
| 10 | Cross-role read visibility under fixtures | Suspected (verified gap) | Under transactional fixtures a tenant's `:reading` pool is a distinct connection from its `:writing` pool and is never connection-shared (AR's `setup_shared_connection_pool` collapses only same-shard pools; apartment's tenant pools live on per-`tenant:role` shards). So a read through `:reading` does not see the `:writing` pool's uncommitted in-test writes (probe: 3 rows written under `:writing`, count under `:reading` = 0), and Rails forbids writing through `:reading` outright. In a real primary/replica app on transactional tests, reads-see-writes is expected (the purpose of `setup_shared_connection_pool`); apartment does not arrange it for tenant pools. Surfaced 2026-06 while building the member-5 `:reading` coverage; tracked, not fixed. |

Failure-class members 1–4 are closed; the workstream's escape-hatch question (originally tracked as workstream item 6) is closed docs-only — see Shipped #6. Member 5's role axis (lifecycle per handler) is now closed via a same-database `:reading` role in the integration helper (see Shipped #7 below and `docs/designs/reading-role-test-support.md`); its multi-physical-DB axis stays open, and the work surfaced member 10 (cross-role read visibility under fixtures). **Member 7 is closed** ([`transaction-taint-detection.md`](transaction-taint-detection.md)) — and note it turned out **not** to be a fixture-only member at all, which is why it now carries its own design doc. Failure-class members 8, 9, 10 are tracked but not in scope this iteration.
...
7. **Multi-handler / `:reading` lifecycle coverage** (role axis of member 5). `register_reading_role!` in the integration helper registers a same-database `:reading` default pool; `fixture_pool_lifecycle_spec.rb` gains a `:reading` context (read-based — Rails forbids writes through `:reading`) and a both-roles example proving a per-tenant `:reading` pool pins, trips the guard, and rebuilds with fresh identity per handler, coexisting with the `:writing` pool as a distinct, independently-pinned object. The earlier "deferred until the dummy app gains read replicas" framing named the wrong surface: the spec runs over the programmatic `V4IntegrationHelper`, not the dummy app, and the role axis needs no replication. Design: `docs/designs/reading-role-test-support.md`. Residual axes (multi-physical-DB, cross-role read visibility) stay in Eventually #7 / member 10.

Downstream cleanup that becomes delete-candidates with these guards in place: the `reset_tenant_pools!` call in cross-tenant test cleanup, the companion best-effort `ROLLBACK` loop guarding `PQTRANS_INERROR` taint (**delete this one on its own merits — it is actively harmful; see member 7**), and manual eager-load stubs added to work around the resulting query pollution. Consumer removals are on their own timeline. Kept-on-purpose: around-hook helpers like `with_tenants` that define a tenant set without cycling pools mid-test — these are ergonomics, not workarounds.

## Eventually (>1mo, deferred)

7. **Residual `:reading` axes** (member 5 role axis is shipped — see Shipped #7). Two pieces remain deferred: (a) the **multi-physical-DB axis** — a `:reading` role whose `pool_config` carries a different `database` key (`connects_to database: { writing:, reading: }`), cheaply approximated by a second database name on the same server; and (b) **cross-role read visibility** under fixtures (member 10) — making a tenant `:reading` pool see the `:writing` pool's in-test writes, which apartment does not currently arrange. Neither is needed for the lifecycle invariant; both await adopter-reported need.
...
- **`PQTRANS_INERROR` taint** (failure-class member 7) — ✅ **SHIPPED. Design: [`transaction-taint-detection.md`](transaction-taint-detection.md).**

  **Two claims in the 2026-07-12 draft of this bullet were wrong, and the corrections are the design.** It recorded the failure as **test-only**, reasoning from the fact that the adopter's `ROLLBACK` loop appears only in their test cleanup. That inference does not hold: their *workaround* is test-only, but the *failure* is not. A poisoned connection passes ActiveRecord's health check (`active?` probes with an empty query, which does not error in an aborted transaction) and is served to the next caller, so under pool-per-tenant a tenant can be dead on a worker until the process restarts. It also scoped the fix as **a recovery path in `Tenant.switch`'s ensure block**, which is the wrong seam: `switch` cannot observe the production failure, misses `switch!` / `reset` / `each`, and would couple a SQL-free context swap to pool internals.

  Shipped instead as a **detect-and-heal at pool checkin** on Apartment-owned tenant pools, PostgreSQL-only, with fixture-pinned connections skipped by construction (they never reach `checkin`). The adopter's savepoint regression is explained and recorded: a raw `ROLLBACK` silently destroys the *enclosing* transaction, which is why it must never ship and why their loop is a delete-candidate rather than a pattern to adopt.
...
- **Within-process thread / job boundaries** (failure-class member 9) — ✅ **CLOSED (2026-07-13): audited clean, then refuted.** Design: [`out-of-band-tenant-ddl.md`](out-of-band-tenant-ddl.md).

  The **threading** hazard was audited clean (2026-07-12): the adopter runs **none** of the risky patterns — Sidekiq is faked in specs (the two inline opt-ins run synchronously on the example's own thread), CI shards across **OS processes** rather than `parallel_tests` threads, and there are **zero** occurrences of `Tenant.switch`/`switch!` inside `Thread.new`, `Concurrent::Future`, or similar. Their only async executor never touches ActiveRecord. Nothing has bitten — from a codebase that would have surfaced it.

  The **out-of-band DDL** hazard it was rescoped to was then **refuted by live-database probes**: a warm pool survives an external drop-and-recreate on every engine (`search_path` is a name re-resolved per query, not a baked handle; a killed backend self-heals on AR's next checkout). No eviction helper ships. The probes did expose a real, DDL-unrelated defect — `deregister_shard` and `remove_tenant` were each half of one operation, and either alone was wrong — which is fixed.

  What the audit *did* surface is an **API gap, not a bug — and it is about out-of-band DDL, not threads.** The one pool-aware call site is a restore path: an external tool (a shell `pg_restore`) drops and recreates a tenant's schema outside ActiveRecord entirely. The tenant's warm v4 pool still holds a connection with a baked `search_path` and a schema cache of now-dead table OIDs, so the next `switch` raises `PG::UndefinedTable`. It hand-rolls a two-step eviction — `Apartment.pool_manager.remove_tenant(tenant)`, then `Apartment.deregister_shard(key)` per removed pool — to recover.

  That two-step is the leaky seam, and the gem already contains its twin: `Apartment::Migrator#evict_migration_pools` (`lib/apartment/migrator.rb:221`) does the same `remove` + `deregister_shard` pair, scoped by **role**. W3 is largely promoting it to public API scoped by **tenant** — and drawing an explicit line against the shipped `Tenant.reload_schema_cache!` (member 8), which clears caches but cannot fix a baked `search_path` or a recreated schema. Shape the contract around **out-of-band tenant DDL**, not the threading hazard we hypothesized.
...
- `ActiveRecord::Base.connection_handler.connection_pool_list(:writing)` — the pools `setup_transactional_fixtures` snapshots and pins *eagerly* (verified against ActiveRecord 8.0/8.1 `test_fixtures.rb`). `:reading` and any lazily-created pools are NOT in this snapshot; they enroll only through the `!connection.active_record` subscriber, which pins by the current role. This `:writing`-eager / `:reading`-lazy asymmetry is the per-handler risk member 5 names. Read-only from Apartment's side; do not mutate.
