...
| A different catalog behind the same endpoint name (blue/green on logical replication, restore into the same name) | The reconnect rebuilds, so it self-heals. | The catalog identity in the key differs, so nothing is adopted and the map is rebuilt; the superseded entry is dropped. Without that component this would be the patch's one silent-wrong-cast regression, since fresh catalogs restart type OIDs at 16384. |
| A promoted physical replica behind the same endpoint name | Full rebuild. | Rebuilds once, because the postmaster start time differs. Adoption would have been safe (the catalog is byte-identical); paying one build is the accepted cost of a fail-closed identity. |
| Database restarted, same catalog | Full rebuild. | Rebuilds once per process, and the previous entry is superseded rather than stranded. |
| Identity probe fails (statement timeout, aborted transaction) | n/a | Warns, detaches from any adopted map, and falls back to upstream's per-connection build. Never clears a map other connections hold. |
