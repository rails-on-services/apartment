...
- **W4 — PgBouncer libpq `options` (approach 1)** — ⚠️ **PREMISE REFUTED by the spike (2026-07-12); awaiting re-scope.** The `ruby-pg`/PG-16/18 spike this workstream called for was run, and it killed approach 1: PgBouncer **rejects** the `options` startup packet unless `search_path` is in `track_extra_parameters`, and once it is, v4's existing `SET`-based path is already safe and multiplexed — so `options` is dominated and **should not be built**. The spike also found what actually governs correctness: `search_path` was not reported to clients before **PostgreSQL 18** (so the "requires Citus 12+" note above is obsolete), and on **PG ≤ 17 transaction mode cannot be made safe at all**. Most urgently, the failure mode is not pinning but a **silent cross-tenant read**. Full matrix and re-scope proposal: [`w4-pgbouncer-libpq-spike.md`](w4-pgbouncer-libpq-spike.md). The user-facing warning shipped separately ([`../connection-poolers.md`](../connection-poolers.md)); what remains of W4 is a supported-configuration statement + a PgBouncer CI job, not an M–L code long pole.
